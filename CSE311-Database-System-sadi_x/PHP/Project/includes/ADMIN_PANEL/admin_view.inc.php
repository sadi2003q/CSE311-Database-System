...
function display_deletion_requests(): void {
   $pdo = require_once "../includes/dbh.inc.php";
    ...
    $statusCondition = $status === 'all' ? "" : "WHERE dr.status = :status";
    
    // Order by date
    $order = isset($_GET['order']) && $_GET['order'] === 'oldest' ? "ASC" : "DESC";
    
    $query = "SELECT dr.request_id, u.username, u.email, dr.request_date, dr.reason, dr.status
              FROM DELETION_REQUESTS dr
              JOIN USERS u ON dr.user_id = u.user_id
              $statusCondition
              ORDER BY dr.request_date $order
              LIMIT :limit OFFSET :offset";
    ...
    if (empty($requests)) {
        echo "<tr><td colspan='7'>No deletion requests found</td></tr>";
        return;
    }
    ...
    foreach ($requests as $request) {
        ...
        echo "<td>" . date('M j, Y', strtotime($request['request_date'])) . "</td>";
        echo "<td>" . htmlspecialchars($request['reason']) . "</td>";
        echo "<td class='$statusClass'>" . $request['status'] . "</td>";
        echo "<td>";
        
        if ($request['status'] === 'pending') {
            echo "<button class='action-btn approve-btn' onclick='approveRequest(" . $request['request_id'] . ")'>Approve</button>";
            echo "<button class='action-btn reject-btn' onclick='rejectRequest(" . $request['request_id'] . ")'>Reject</button>";
        }
        
        echo "<button class='action-btn view-btn' onclick='viewUser(" . $request['user_id'] . ")'>View User</button>";
        echo "</td>";
        echo "</tr>";
    }
}